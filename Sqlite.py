...
def updateData():
    if txtName.get() == "" or txtAge.get() == "" or txtAdd.get() == "" or cb.get() == "" or txtCon.get() == "" or txtMail.get() == "":
        messagebox.showinfo("Message", "Please Fill All Records")
    # elif (re.fullmatch(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b',txtMail.get())):
    #      messagebox.showinfo("Message", "Please Enter Valid Email")
    #
    # elif ((phonenumbers.is_valid_number(phonenumbers.parse(txtCon.get()))) != "true"):
    #
    #       messagebox.showinfo("Message", "Please Enter Valid Phone Number")
    else:
        data= db.update_record(txtName.get(), txtAge.get(), cb.get(), txtAdd.get(), txtCon.get(), txtMail.get(), (row[1]))
        fetchData()
        clearData()
        dat=[]
        dat.append(data)
        ...
        sname=str(xy[0].strip())
        newname=re.sub(r"[^a-zA-Z0-9]","",sname)

        age = str(xy[1].strip())
        newage=re.sub(r"[^0-9]","",age)

        Gender = xy[2].strip()
        newgender = re.sub(r"[^a-zA-Z]", "", Gender)

        Address = xy[3].strip()
        newadd = re.sub(r"[^a-zA-Z0-9]", "", Address)

        Contact = xy[4].strip()
        newcontct = re.sub(r"[^0-9]", "", Contact)

        Mail = xy[5].strip()
        newmail = re.sub("^[a-z0-9]+[\._]?[a-z0-9]+[@]\w+[.]\w{2,3}$", "", Mail)

        messagebox.showinfo("Message", "Record Update Successfully")
        

        p = subprocess.Popen(["powershell.exe", "& 'C:\\Users\\Nuthi Bala Krishna\\Desktop\\mail.ps1' -name " + str(newname) +" -age " + str(newage) +" -gender " + str(newgender) +" -address " + str(newadd) +" -contact " + str(newcontct)+"  -email " + str(newmail)+""],stdout=sys.stdout)
        p.communicate()
